...
def read_csv():
    ...
    '''
    df = pd.read_csv("../dataset/valid_recipe_dataset.csv") #convert csv to dataframe
    ...
    with open("full_recipe_cache.pkl", "wb") as f: #save full recipe dictionary
        pickle.dump(ds, f)

def reduce_ds(ingredients:list):
    '''
    Takes a dictionary of ingredients and reduces the recipe dictionary to those that only use the valid ingredients.
    ...
    with open("reduced_recipe_cache.pkl", "wb") as f: #save full recipe dictionary
        pickle.dump(ds, f)
...
ingredients = ["apple", "blueberries", "carrot", "strawberries", "broccoli"]
